...
export function Select<TFieldValues extends FieldValues = FieldValues>(props: SelectProps<TFieldValues>) {
  ...
  const { label, placeholder = 'Selecione', helperText, error, errorText, options = [], sections = [], value, defaultValue, onValueChange, multiple = false, disabled, fullWidth = true, outsideRefs = [], searchable = false, searchValue = '', searchPlaceholder = 'Buscar...', onSearchChange, className, containerClassName, 'aria-label': ariaLabel } = props
  ...
  return <div ref={rootRef} className={cn('relative space-y-2', fullWidth ? 'w-full' : 'w-fit', containerClassName)}>
    ...
    <button id={id} type="button" role="combobox" value={String(Array.isArray(current) ? current.join(',') : current)} aria-label={ariaLabel} disabled={disabled} aria-expanded={open} onClick={() => setOpen((current) => !current)} className={cn('flex min-h-12 items-center justify-between rounded-xl border border-black/10 bg-white/45 px-4 text-left text-sm text-[#151A21] outline-none transition focus:border-sky-400 disabled:cursor-not-allowed disabled:opacity-55 dark:border-white/10 dark:bg-black/25 dark:text-white', fullWidth ? 'w-full' : 'w-fit min-w-40', className)}>
    ...
    {open ? <div role="listbox" className="absolute z-50 mt-0.5 max-h-64 w-full overflow-auto rounded-xl border border-white/55 bg-white/35 p-2 shadow-xl backdrop-blur-xl backdrop-saturate-150 dark:border-white/10 dark:bg-black/35">{searchable ? <div className="relative mb-2"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 opacity-60" /><input autoFocus value={searchValue} onChange={(event) => onSearchChange?.(event.target.value)} placeholder={searchPlaceholder} aria-label={searchPlaceholder} className="h-10 w-full rounded-lg border border-black/10 bg-white/35 pl-9 pr-9 text-sm outline-none focus:border-sky-400 dark:border-white/10 dark:bg-black/20 dark:text-white" />{searchValue ? <button type="button" aria-label="Limpar busca" onClick={() => onSearchChange?.('')} className="absolute right-2 top-1/2 -translate-y-1/2 p-1"><X className="size-4 opacity-70" /></button> : null}</div> : null}<div className="space-y-1">{sections.map((section) => <div key={String(section.label)} className="space-y-1"><p className="px-3 py-2 text-xs font-medium opacity-60">{section.label}</p>{section.options.map((option) => <Option key={option.value} option={option} selected={selected.includes(option.value)} onSelect={choose} />)}</div>)}{options.map((option) => <Option key={option.value} option={option} selected={selected.includes(option.value)} onSelect={choose} />)}</div></div> : null}
