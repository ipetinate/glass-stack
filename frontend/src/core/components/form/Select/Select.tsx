...
export type SelectProps<TFieldValues extends FieldValues = FieldValues> = { label?: ReactNode; placeholder?: string; helperText?: ReactNode; error?: ReactNode; errorText?: ReactNode; options?: SelectOption[]; sections?: SelectSection[]; value?: string | string[]; defaultValue?: string | string[]; onValueChange?: (value: string | string[]) => void; multiple?: boolean; disabled?: boolean; fullWidth?: boolean; outsideRefs?: RefObject<HTMLElement | null>[]; className?: string; containerClassName?: string; 'aria-label'?: string; control?: Control<TFieldValues>; name?: FieldPath<TFieldValues>; rules?: RegisterOptions<TFieldValues> }
...
export function Select<TFieldValues extends FieldValues = FieldValues>(props: SelectProps<TFieldValues>) {
  ...
  const { label, placeholder = 'Selecione', helperText, error, errorText, options = [], sections = [], value, defaultValue, onValueChange, multiple = false, disabled, fullWidth = true, outsideRefs = [], className, containerClassName, 'aria-label': ariaLabel } = props
  ...
  return <div ref={rootRef} className={cn('relative space-y-2', fullWidth ? 'w-full' : 'w-fit', containerClassName)}>
    ...
    {open ? <div role="listbox" className="absolute z-50 mt-0.5 max-h-64 w-full overflow-auto rounded-xl border border-black/10 bg-white/85 p-2 shadow-xl backdrop-blur-xl backdrop-saturate-150 dark:border-white/15 dark:bg-[#151A21]/90">{sections.map((section) => <div key={String(section.label)} className="mb-1 last:mb-0"><p className="px-3 py-2 text-xs font-medium opacity-60">{section.label}</p>{section.options.map((option) => <Option key={option.value} option={option} selected={selected.includes(option.value)} onSelect={choose} />)}</div>)}{options.map((option) => <Option key={option.value} option={option} selected={selected.includes(option.value)} onSelect={choose} />)}</div> : null}
