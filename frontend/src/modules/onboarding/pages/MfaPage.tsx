...
export function MfaPage() {
  ...
  return <form id="onboarding-mfa-form" onSubmit={submit}><OnboardingStage><OnboardingStageTitle>Proteja sua conta administradora</OnboardingStageTitle><div className="mt-8 grid items-start gap-8 sm:grid-cols-[240px_minmax(0,1fr)]"><img src={state.enrollment.qrCodeDataUri} alt="QR code para configurar autenticação" className="size-60 rounded-2xl bg-white p-3" /><div className="min-w-0"><p className="text-lg">Escaneie o QR code com seu aplicativo autenticador.</p><code className="mt-5 block break-all rounded-lg bg-white/35 p-3 text-xs dark:bg-[#111c2b]/55">{state.enrollment.secret}</code><GlassInput label="Código de 6 dígitos" containerClassName="mt-6 max-w-xs" value={state.totpCode} onChange={(e) => state.setField('totpCode', e.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" /></div></div>{state.error ? <StageError>{state.error}</StageError> : null}</OnboardingStage></form>
}
